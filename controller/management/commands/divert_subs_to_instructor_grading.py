...
class Command(BaseCommand):
    ...
    help = "Usage: divert_subs_to_instructor_grading <course_id> --dry-run \n" \
    ...
    def handle(self, *args, **options):
        ...
        if len(args) == 1:
            ...
            return
        try:
            reset_and_divert_failed_basic_check_submissions(course_id,
                                                            dry_run=dry_run)
        except Exception as ex:
            print "ERROR: {0}".format(ex.message)


def reset_and_divert_failed_basic_check_submissions(course_id, dry_run=True):
    """
